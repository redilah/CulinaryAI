  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto flex flex-col items-center justify-center py-20 space-y-4">
        ...
        <p className="text-emerald-600 font-black uppercase tracking-widest text-sm animate-pulse">Menyusun Jadwal Makan...</p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <header className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500">Strategi zero-waste menggunakan bahan yang hampir kedaluwarsa.</p>
        </div>
        <button onClick={onRefresh} className="text-emerald-600 font-bold text-xs uppercase tracking-widest hover:text-emerald-700">
          Ulangi <i className="fa-solid fa-rotate-right ml-1"></i>
        </button>
      </header>

      <div className="space-y-6">
        {plan.map((day, idx) => (
          <div key={idx} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                { label: 'Sarapan', menu: day.breakfast, icon: 'fa-sun' },
                { label: 'Makan Siang', menu: day.lunch, icon: 'fa-cloud-sun' },
                { label: 'Makan Malam', menu: day.dinner, icon: 'fa-moon' },
              ].map(meal => (
                <div key={meal.label} className="bg-slate-50 p-5 rounded-2xl space-y-2 border border-slate-100/50 shadow-inner">
            ...
            <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl flex gap-3 items-center">
              ...
              <p className="text-xs text-emerald-800 font-medium italic">{day.reason}</p>
            </div>
          </div>
        ))}
      </div>
