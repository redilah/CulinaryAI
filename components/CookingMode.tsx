    ...
    <div className="fixed inset-0 bg-white z-50 flex flex-col md:flex-row overflow-hidden font-sans text-slate-900">
      ...
      <div className="hidden md:flex w-full h-full flex-row">
        <div className="w-[30%] bg-slate-900 text-white p-10 flex flex-col h-full overflow-y-auto">
          <button onClick={onExit} className="mb-10 text-slate-400 font-black text-xs uppercase tracking-widest hover:text-white flex items-center gap-3">
          ...
          <div className="bg-white/5 rounded-[2rem] p-8 border border-white/10 shadow-inner mb-6">
            <header className="flex items-center justify-between mb-6">
              ...
              <div className="flex bg-white/10 p-1 rounded-xl">
                <button onClick={() => setLang('ID')} className={`px-3 py-1 text-[9px] font-black rounded-lg ${lang === 'ID' ? 'bg-white text-slate-900 shadow-sm' : 'text-white/40'}`}>ID</button>
                <button onClick={() => setLang('EN')} className={`px-3 py-1 text-[9px] font-black rounded-lg ${lang === 'EN' ? 'bg-white text-slate-900 shadow-sm' : 'text-white/40'}`}>EN</button>
              </div>
            </header>
        ...
        <div className="flex-1 flex flex-col p-16 bg-white overflow-y-auto">
          <div className="max-w-4xl mx-auto w-full flex-1 flex flex-col">
            ...
            <div className="flex gap-6 mt-auto">
              {currentStep > 0 && (
                <button onClick={() => setCurrentStep(prev => Math.max(0, prev - 1))} className="flex-1 py-6 bg-slate-100 rounded-[2rem] font-black text-xs uppercase tracking-widest hover:bg-slate-200">
                  {t.prev}
                </button>
              )}
              <button 
                onClick={() => currentStep < instructions.length - 1 ? setCurrentStep(prev => prev + 1) : onExit()} 
                className={`py-6 bg-emerald-600 text-white rounded-[2rem] font-black text-xs shadow-2xl uppercase tracking-widest ${currentStep > 0 ? 'flex-[2]' : 'w-full'}`}
              >
      ...
      <div className="md:hidden flex flex-col h-full bg-slate-50">
        <div className="p-6 bg-white flex items-center border-b border-slate-100 shrink-0">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-3 bg-slate-50 rounded-2xl text-slate-800 font-black uppercase text-[10px] tracking-widest active:scale-95 transition-transform"
          >
        ...
        <div className="flex-1 relative overflow-hidden">
          <div className="flex h-full w-[200%] transition-transform duration-500 ease-out" style={{ transform: mobileSlide === 'overview' ? 'translateX(0%)' : 'translateX(-50%)' }}>
            ...
            <div className="w-[50%] h-full overflow-y-auto p-6 space-y-6 flex flex-col">
              ...
              <button onClick={() => setMobileSlide('execution')} className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-emerald-100">
                {t.start}
              </button>
            </div>
            ...
            <div className="w-[50%] h-full flex flex-col p-6 bg-white overflow-y-auto">
              ...
              {/* Step instructions at Top */}
              <div className="mb-10 min-h-[140px] flex items-center justify-center text-center px-4">
              ...
              <div className="flex justify-center gap-6 mb-10">
                <button onClick={() => speakText(instructions[currentStep])} className="w-16 h-16 bg-slate-50 text-slate-800 rounded-[1.5rem] flex flex-col items-center justify-center shadow-sm active:bg-slate-100">
                  ...
                  <span className="text-[7px] font-black uppercase mt-1">Speak</span>
                </button>
                <button onClick={handleToggleLive} className={`w-16 h-16 rounded-[1.5rem] flex flex-col items-center justify-center text-white shadow-md active:scale-95 ${isLiveActive ? 'bg-rose-500 animate-pulse' : 'bg-slate-900'}`}>
              ...
              {/* Navigation sticky bottom area */}
              <div className="mt-auto pt-4 flex gap-4">
                {currentStep > 0 && (
                  <button 
                    onClick={() => setCurrentStep(prev => Math.max(0, prev - 1))}
                    className="flex-1 py-5 bg-slate-100 text-slate-600 rounded-2xl font-black uppercase tracking-widest text-[10px]"
                  >
                    {t.prev}
                  </button>
                )}
